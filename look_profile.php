...
<table cellpadding="7">
    <td>
        <form action="delete_user.php?id=<?= $_GET['id'] ?>"  method="post" enctype="multipart/form-data">
            <button>Удалить данный профиль</button>
        </form>
    </td>
</table>
...
foreach ($all_Image as $image
    ...
    {
        if($image['id'] === $value['image_id'] && $_GET['id'] === $value['user_id']){
            $price = $image['price'];
...
foreach ($all_Image as $image) {
    ...
    <table align="center" border="2" bordercolor="Blue">
        ...
        <tr>
            <td>
                <?php $imageIdUserId = $image['id'] . " " . $_GET['id']; ?>
                <p align="center"><a href="delete_personal_form.php?imageIdUserId=<?= $imageIdUserId ?>">Изъять</a></p>
            </td>
        </tr>
        <?php
        foreach ($personal_stock as $number) {
            if ($number['image_id'] === $image['id'] && $number['user_id'] === $_GET['id']) {
                ?>
                <tr>
                    <td>
                        <?= $number['number_have']; ?>
